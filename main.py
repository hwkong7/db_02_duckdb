...
def main(page: ft.Page):
    ...
    sticky_header = ft.Row(
        controls=[
            ft.Container(
                ft.Text(col.upper(), weight="bold", size=16), expand=lengths[col]
            )
            for col in df.columns
        ],
    )
        ...
        for _, row in current_df.iterrows():
            rows.append(
                ft.Container(
                    content=ft.Row(
                        [
                            ft.Text(str(row[col]), expand=lengths[col])
    ...
    scrollable_data = ft.Column(
        controls=create_rows(df), scroll=ft.ScrollMode.ALWAYS, expand=True
    )
    ...
    page.add(filter_input, sticky_header, ft.Divider(), scrollable_data)
